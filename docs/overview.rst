...
After filtering, aggregation and data transnformation the data will be delivered to the output. Several data consumers can receive data from one Kafka Topic.
...
If this feature is used, the subscribed data will be aggregregated and will be dlivered in chunks.

The data will then be wrapped in a Json array in the same fasion as DMaap does. The type configuration can define if the received data is Json.
